...
<script lang="ts">
    ...
    export let usePopup = false;
    const cBase = "textarea cursor-pointer";
    const cInterface = "space-y-4";
    const cChipList = "flex flex-wrap gap-2 flex-col";
                ...
                <div class="input-chip-list {classesChipList}" transition:fly|local={{ duration, opacity: 0, y: -20 }}>
                    {#each chipValues as { id, val }, i (id)}
                        ...
                        <div animate:flip={{ duration }}
                            ...
                            <button
                                type="button"
                                class="chip max-w-full hover:max-w-none {chips}"
                                on:click={(e) => {
                                    removeChip(e, i, val);
                                }}
                                on:click
                            ...
                            >
                                <span class="text-ellipsis overflow-hidden">✕ {val}</span>
