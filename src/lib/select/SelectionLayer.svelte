<script lang="ts">
    ...
    const createGridConfigStore = (element:DashboardElement, uuid:string) => {
        return derived(
            [selectedStore, element.type, element.layout.x, element.layout.y, element.layout.height, element.layout.width], 
            ([selected, type, x, y, height, width]) => {
                console.log(type, x, y, width, height)
                return {x: (x??1)-1, y: (y??1)-1, h:height, w:width, 
                    fixed:(selected !== uuid),
                    draggable:(selected === uuid),
                    resizable:(selected === uuid),
                    id:uuid,
